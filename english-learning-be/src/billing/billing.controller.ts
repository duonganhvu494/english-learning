...
export class BillingController {
  ...
  async getMyBillingSubscription(
    @Req()
    req: AuthRequest,
  ): Promise<ApiResponse<BillingSubscriptionResponseDto | null>> {
  ...
  async startMyBillingSubscription(
    @Body()
    dto: StartBillingSubscriptionDto,
  ...
  @Patch("me/subscription/plan")
  async changeMyBillingPlan(
    @Body()
    dto: ChangeBillingPlanDto,

    @Req()
    req: AuthRequest,
  ): Promise<ApiResponse<BillingSubscriptionResponseDto>> {
    const result = await this.billingService.changeMyBillingPlan(
      req.user.userId,
      dto.planCode,
    );
