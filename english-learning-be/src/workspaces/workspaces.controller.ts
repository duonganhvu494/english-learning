...
export class WorkspacesController {
  ...
  async createWorkspace(
    ...
    const workspace = await this.service.createWorkspace(dto, req.user.userId);
  ...
  async getMyWorkspace(
    ...
    const result = await this.service.getMyWorkspace(req.user.userId);
  ...
  async getMyWorkspaceSubscription(
    @Req() req: AuthRequest,
  ): Promise<ApiResponse<WorkspaceSubscriptionResponseDto>> {
  ...
  async listStudents(
    ...
    const students = await this.service.listWorkspaceStudents(workspaceId);
  ...
  async getStudentDetail(
    @Param("id") workspaceId: string,
    @Param("studentId") studentId: string,
  ): Promise<ApiResponse<StudentDetailResponseDto>> {
    const result = await this.service.getStudentDetail(workspaceId, studentId);

    return ApiResponse.success(result, "Workspace student detail retrieved");
  }
